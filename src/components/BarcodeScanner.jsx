...
// シンプル版: バーコードを見つけたら即返す。前回JANの保持/禁止ロジックは一切なし。
  ...
  try {
    if (deviceId) {
      return await navigator.mediaDevices.getUserMedia({
        audio: false,
        video: { deviceId: { exact: deviceId }, ...base },
      });
    }
  } catch {}
  try {
...
export default function BarcodeScanner({ open, onClose, onDetected }) {
  ...
  const start = useCallback(
    async (explicitId) => {
      ...
      try {
        setDevices(await enumerateBackCameras());
      } catch {}
      ...
      if (caps.zoom) {
        const init = Math.max(
          ...
          Math.min(
            caps.zoom.max ?? 1,
            (caps.zoom.min ?? 1) + ((caps.zoom.max ?? 1) - (caps.zoom.min ?? 1)) * 0.15
          )
        );
        await applyZoom(init);
      }

      // 4) スキャナ（Detector優先 → ZXing）
    ...
    (async () => {
      try {
        ...
        if (name === "NotAllowedError" || name === "SecurityError")
          setErrorMsg("カメラが『拒否』です。設定でこのサイトのカメラを『許可』にしてください。");
        else if (name === "NotFoundError" || name === "OverconstrainedError")
          setErrorMsg(
            "背面カメラが見つかりません。端末再起動または別ブラウザをお試しください。"
          );
        else if (name === "NotReadableError")
          setErrorMsg("他アプリがカメラ使用中の可能性。全て終了後に再試行してください。");
  ...
  useEffect(() => {
    ...
    const i = setInterval(() => {
      ...
      setHud(
        `${v?.readyState ?? "-"} ${v?.videoWidth ?? 0}x${v?.videoHeight ?? 0} zoom:${(s.zoom ?? zoomVal ?? "").toString()}`
      );
    }, 500);
