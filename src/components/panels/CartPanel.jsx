// ------------------------------------------------------------
// CartPanel（完全差し替え版・堅牢化 / Hooks依存を厳密化）
...
// ・ESLint(exhaustive-deps)対応：fmtはuseCallback, btnMiniStyleはuseMemo化
// ------------------------------------------------------------
...
function sanitizeLine(raw, idx) {
  ...
  const sku =
    ...
    raw.variantId ||
    raw.merchandiseId ||
    (raw?.merchandise && raw.merchandise.id) ||
    "";
  const title = raw.productTitle || raw.title || "(無題)";
  const key = raw.id || (origin && sku ? `${origin}:${sku}` : null) || `ln-${idx}`;

  const qty = Number.isFinite(raw.quantity) ? raw.quantity : 1;
  const lineAmount = Number.isFinite(Number(raw.lineAmount))
  ...
  const variantTail =
    typeof merchandiseId === "string" && merchandiseId.includes("/")
      ? merchandiseId.split("/").pop()
      : raw.variantId || "-";

  const availableForSale =
    typeof raw.availableForSale === "boolean"
...
export default function CartPanel({ isOpen, onClose }) {
  ...
  const openCartPage = useCallback(async () => {
    setCheckingOut(true);
    try {
  ...
  const openCheckoutDirect = useCallback(async () => {
    setCheckingOut(true);
    try {
  ...
  const listContent = useMemo(() => {
    try {
      ...
      return safe.map((ln, idx) => {
        ...
        const disableQty =
          ln.origin === "staged" && typeof ln.syncState === "string" && ln.syncState.startsWith("error");
              ...
              <div style={{ fontSize: 14, fontWeight: 600 }}>
                ...
                {ln.origin === "staged" && ln.syncState && (
                  <span style={{ marginLeft: 6, fontSize: 10, color: "#a33" }}>
                    {ln.syncState === "error_no_variant"
                      ? "未登録商品（要登録）"
                      : ln.syncState === "error_oos"
                      ? "在庫不足"
                      : ""}
                  </span>
                )}
  ...
  }, [lines, updateQty, removeLine, fmt, btnMiniStyle, badge]);
  ...
  return (
    ...
    >
      ...
      <div style={{ padding: "0 14px 6px", fontSize: 13, lineHeight: 1.4, color: "#333" }}>
        <div>
          確定小計（オンライン）：<b>{fmt(onlineSubtotal || 0)}</b>
        </div>
        {stagedSum > 0 && <div style={{ color: "#666" }}>推定小計（在庫確認前）：{fmt(stagedSum)}</div>}
        {stagedSum > 0 && (
      ...
      >
        <div style={{ fontSize: 14 }}>
          小計：<b>{fmt(subtotal)}</b>
          {hasPending && (
            <span style={{ marginLeft: 8, fontSize: 12, color: "#a00" }}>（未同期の商品があります）</span>
          )}
        </div>
